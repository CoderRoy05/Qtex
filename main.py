...
client = Quotex(
    email="royalalawode@gmail.com",
    password="captainroy17",
)
...
async def trade_and_check_win():
    ...
    if check_connect:
        ...
        if asset_open[2]:
            ...
            if status:
                print(colored("[INFO]: ", "blue"), "Waiting for result...")
                # Check if 'id' key exists in trade_info dictionary
                if 'id' in trade_info:
                    if await client.check_win(asset, trade_info["id"]):
                        print(colored("[INFO]: ", "green"), f"Win -> Profit: {client.get_profit()}")
                    else:
                    ...
                    print(colored("[WARN]: ", "yellow"), "Key 'id' not found in trade_info.")
            else:
                print(colored("[WARN]: ", "light_red"), "Operation not realized.")
        else:
            print(colored("[WARN]: ", "light_red"), "Asset is closed.")
